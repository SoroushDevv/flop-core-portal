...
export const TeamHub: React.FC = () => {
  ...
  const handleJoinSquad = async (squadId: string) => {
    ...
    if (squad.members.length >= squad.maxSlots) {
      alert("This squad is already full (maximum limit reached).");
      return;
    }

    if (squad.members.some((m) => m.did === userDid.trim())) {
      alert("Your DID is already registered in this squad.");
      return;
    }
    ...
    const updated = squads.map((s) => {
      if (s.id === squadId) {
        return {
          ...s,
          members: [...s.members, { did: userDid.trim(), xAccount: userX.trim() || undefined }],
        };
      }
      return s;
    });
  ...
  const handleCreateSquad = (e: React.FormEvent) => {
    ...
    if (!newTeamName.trim() || !newGameId.trim() || !userDid.trim()) {
      alert("Please provide Team Name, Game ID, and your DID.");
      return;
    }
    ...
    const newSquad: Squad = {
      ...
      maxSlots: 8,
      members: [
        { did: userDid.trim(), xAccount: userX.trim() || undefined, isLeader: true },
      ],
    };
