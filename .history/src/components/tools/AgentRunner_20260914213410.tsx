...
export const AgentRunner: React.FC = () => {
  ...
  const handleStart = () => {
    if (!did.trim().startsWith("did:key")) {
      alert("Please enter a valid did:key string before launching daemon.");
      return;
    }

    setIsRunning(true);
    addLog(`Initializing FlopCore client agent daemon...`, "info");
    addLog(`Signed identifier bound: ${did.trim()}`, "success");
    addLog(`Subscribed to corridor stream: #${corridorRoom}`, "info");

    executeHeartbeatCycle(did, corridorRoom);
