...
export const SonnetPortal: React.FC = () => {
  ...
  const dispatchPayload = async (roomName: string, payload: Record<string, unknown>) => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        botSpeak(`Corridor #${roomName} confirmed receipt: ${data.request_id || "OK"}`, "success");
      } else {
        appendLog(`Error from referee: ${data.error || "Execution failed"}`);
        botSpeak(`Referee rejection on #${roomName}: ${data.error || "Integrity failed"}`, "error");
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Network error";
      appendLog(`Failed to communicate with referee: ${msg}`);
      botSpeak(`Corridor dispatch failed: ${msg}`, "error");
    } finally {
  ...
  const handleVoterRegister = () => {
    if (!did.trim().startsWith("did:key:")) {
      botSpeak("Voter DID is missing or invalid. Format: did:key:z6Mk...", "error");
      return;
    }
  ...
  const handleVoterBallot = () => {
    if (!did.trim().startsWith("did:key:")) {
      botSpeak("Please provide a valid voter DID.", "error");
      return;
    }
    if (!entryId.trim()) {
      botSpeak("Entry ID is required to cast a ballot vote.", "warning");
      return;
    }
  ...
  const handleWriterRegister = () => {
    if (!did.trim().startsWith("did:key:")) {
      botSpeak("Writer DID is missing or invalid.", "error");
      return;
    }
    if (!xUsername.trim()) {
      botSpeak("Official rules require your X username for writer registration.", "warning");
      return;
    }
  ...
  const handleRoomRequest = () => {
    if (!did.trim().startsWith("did:key:")) {
      botSpeak("Valid DID required to request a team room.", "error");
      return;
    }
    if (!gameId.trim()) {
      botSpeak("Game ID cannot be empty.", "warning");
      return;
    }
  ...
  const handleSignRoster = () => {
    ...
    if (membersList.length < 4 || membersList.length > 8) {
      botSpeak(`Roster rule violation: Team must contain between 4 and 8 DIDs (found ${membersList.length}).`, "error", 5000);
      return;
    }
  ...
  const handleWordSubmit = () => {
    if (!wordValidation.isValid) {
      botSpeak(`Forbidden characters! Word contains letters not found in your DID: [${wordValidation.invalidLetters.join(", ")}]`, "error", 5000);
      return;
    }
  ...
  const handleFinalSubmit = async () => {
    if (!poemText.trim()) {
      botSpeak("Canonical 14-line poem text is required for SHA-256 submission.", "warning");
      return;
    }
    if (!xPostId.trim()) {
      botSpeak("X Post ID is required by the contest referee.", "warning");
      return;
    }
