      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`p-5 rounded-xl bg-[#0B0F19]/90 border border-[#162238] ${item.border} transition-all duration-300 backdrop-blur-sm group`}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] text-slate-500 uppercase tracking-wider">
                  {item.detail}
                </span>
                <div className="p-1.5 rounded-lg bg-[#04060A] border border-[#162238]">
